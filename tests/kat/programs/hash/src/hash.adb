...
procedure Hash is
   ...
   type Monte_Carlo_Kind is (None, SHAVS, SHA3VS);
   --  The Monte Carlo tests defined in SHAVS and SHA3VS are slightly
   --  different algorithms and so we need to distinguish between them

   Algo        : Tux.Hashing.Algorithm_Kind;
   File        : Ada.Streams.Stream_IO.File_Type;
   Length      : Natural;
   Buffer      : Tux.Types.Byte_Array (1 .. 4096);
   Monte_Carlo : Monte_Carlo_Kind := None;

begin
   --  Check arguments

   if Argument_Count not in 1 .. 2 then
      Put_Line ("usage: hash ALGO_NAME [--monte-shavs] [--monte-sha3vs]");
      Set_Exit_Status (1);
      return;
   end if;

   if Argument_Count = 2 then
      if Argument (2) = "--monte-shavs" then
         Monte_Carlo := SHAVS;

      elsif Argument (2) = "--monte-sha3vs" then
         Monte_Carlo := SHA3VS;
   ...
   begin
      if Monte_Carlo = None then
         ...
         if Monte_Carlo = SHAVS then
            --  Monte Carlo test as defined in SHAVS
            ...
            for I in 1 .. 1000 loop
               Tux.Hashing.Initialize (Ctx);
               Tux.Hashing.Update (Ctx, A);
               Tux.Hashing.Update (Ctx, B);
               Tux.Hashing.Update (Ctx, C);
               Tux.Hashing.Finish (Ctx, Hash);
            ...
            --  Monte Carlo test as defined in SHA3VS

            Hash := Buffer (1 .. Hash_Length);

            for I in 1 .. 1000 loop
               Tux.Hashing.Initialize (Ctx);
               Tux.Hashing.Update (Ctx, Hash);
               Tux.Hashing.Finish (Ctx, Hash);
            end loop;
         end if;
