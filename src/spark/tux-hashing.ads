...
is
   ...
   type Algorithm_Kind is
     (SHA1,
      ...
      SHA512_224,
      SHA512_256);
   --  Identifies one of the hash functions supported by this interface.
   --
   ...
   --  by the Enabled_Algorithm_Kind type (see below).
   ...
   --  Represents the length of the hashes generated by a hash function
   ...
   --  Lookup table of the hash lengths output by each hash algorithm
   ...
   --  Lookup table of the hash block size of each hash algorithm
   ...
   --
   --  This may be called at any time to begin a new hashing session.
   ...
   --  Finish a hashing session and generate the computed hash.
   --
